...
import (
	...
	_ "hotgo/addons/product/crons"
	"hotgo/addons/product/global"
	_ "hotgo/addons/product/logic"
	_ "hotgo/addons/product/queues"
	"hotgo/addons/product/router"
...
func (m *module) Install(ctx context.Context) (err error) {
	...
	g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		fileName := "./addons/product/sql/install.sql"
...
func (m *module) Upgrade(ctx context.Context) (err error) {
	g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		fileName := "./addons/product/sql/upgrade_" + m.skeleton.Version + ".sql"
		e := m.ImportSql(ctx, fileName)
		if e != nil {
			g.Log().Warningf(ctx, "file: %s Not Found", fileName)
			return nil
...
func (m *module) UnInstall(ctx context.Context) (err error) {
	g.DB().Ctx(ctx).Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		fileName := "./addons/product/sql/uninstall.sql"
		e := m.ImportSql(ctx, fileName)
		if e != nil {
			g.Log().Errorf(ctx, "file: %s Not Found", fileName)
			return nil
		}
		return nil
	})
