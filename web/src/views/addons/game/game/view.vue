<template>
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" title="游戏详情"> <!-- CURD详情页--> </n-card>
    </div>
    <n-card :bordered="false" class="proCard mt-4" size="small" :segmented="{ content: true }">
      <n-descriptions label-placement="left" class="py-2" column="4">
        <n-descriptions-item>
          <template #label>唯一值</template>
          {{ formValue.gameSku }}
        </n-descriptions-item>

        <n-descriptions-item>
          <template #label>游戏全名</template>
          {{ formValue.name }}
        </n-descriptions-item>
...
import {View} from '@/api/addons/game/game';
...
onMounted(async () => {
  ...
  formValue.value = await View({gameId: id});
});
</script>
